...
slack.on(RTM_EVENTS.MESSAGE, message => {
  ...
  let displayName =
    (user != null ? user.display_name : void 0) != null
      ? '@' + user.name
      : 'UNKNOWN_USER';
  if (user && user.is_bot) {
...
function _setVolume(input, channel, userName) {
  ...
  if (isNaN(vol)) {
    ...
    if (vol > maxVolume) {
      _slackMessage(
        "That's a bit extreme, " + userName + '... lower please.',
        channel.id
      );
    } else {
      sonos
    ...
    .then(result => {
      ...
      _currentTrack(channel, function(err, track) {
        if (!result) {
          _log(result);
          _slackMessage(
            'Seems like the queue is empty... Have you tried adding a song?!',
            channel.id
          );
        }
        if (err) {
  ...
  _currentTrackTitle(channel, function(err, track) {
    ...
    if (gongBanned) {
      _slackMessage(
        'Sorry ' +
          userName +
          ', the people have voted and this track cannot be gonged...',
        channel.id
      );
      return;
    }
    ...
    if (gongScore[userName] >= gongLimitPerUser) {
      _slackMessage(
        'Are you trying to cheat, ' + userName + '? DENIED!',
        channel.id
      );
    } else {
      if (userName in voteScore) {
        _slackMessage(
          'Having regrets, ' +
            userName +
            "? We're glad you came to your senses...",
          channel.id
        );
      }
...
function _vote(channel, userName) {
  ...
  _currentTrackTitle(channel, function(err, track) {
    ...
    if (voteScore[userName] >= voteLimitPerUser) {
      ...
      if (userName in gongScore) {
        _slackMessage(
          'Changed your mind, ' + userName + '? Well, ok then...',
          channel.id
        );
      }
      ...
      voteCounter++;
      _slackMessage(
        'This is VOTE ' + voteCounter + '/' + voteLimit + ' for ' + track,
        channel.id
      );
      if (voteCounter >= voteLimit) {
        _slackMessage(
          'This track is now immune to GONG! (just this once)',
          channel.id
        );
        voteCounter = 0;
        voteScore = {};
...
function _votecheck(channel, userName) {
  ...
  _currentTrackTitle(channel, function(err, track) {
    ...
    _slackMessage(
      'VOTE is currently ' + voteCounter + '/' + voteLimit + ' for ' + track,
      channel.id
    );
...
function _gongcheck(channel, userName) {
  ...
  _currentTrackTitle(channel, function(err, track) {
    ...
    _slackMessage(
      'GONG is currently ' + gongCounter + '/' + gongLimit + ' for ' + track,
      channel.id
    );
...
function _help(input, channel) {
  var message =
    ...
    '`gong` : The current track is bad! ' +
    gongLimit +
    ' gongs will skip the track\n' +
    '`gongcheck` : How many gong votes there are currently, as well as who has gonged.\n' +
    '`vote` : The current track is great! ' +
    voteLimit +
    ' votes will prevent the track from being gonged\n' +
...
function _gongplay(input, channel) {
  sonos
    .queueNext('spotify:track:6Yy5Pr0KvTnAaxDBBISSDe')
    .then(success => {
      _log('GongPlay!!');
    })
    .catch(err => {
  ...
  sonos
    ...
    .then(track => {
      ...
      if (err) {
        ...
        var message = `We're rocking out to *${track.artist}* - *${
          track.title
        }* (${pmin}:${psec}/${fmin}:${fsec})`;
        _slackMessage(message, channel.id);
      }
    ...
    .then(state => {
      ...
      if (state === 'stopped') {
        ...
        _addToSpotify(userName, uri, albumImg, trackName, channel, function() {
          if (channel.name === adminChannel) {
            _slackMessage(
              'Sonos is currently PAUSED. Type `resume` to start playing...',
              channel.id
            );
          }
        });
        ...
        _slackMessage(
          "Sonos says it is 'transitioning'. We've got no idea what that means either...",
          channel.id
        );
      } else if (state === 'no_media') {
        _slackMessage(
          "Sonos reports 'no media'. Any idea what that means?",
          channel.id
        );
      } else {
        _slackMessage(
          "Sonos reports its state as '" +
            state +
            "'. Any idea what that means? I've got nothing.",
          channel.id
        );
      }
    })
...
function _search(input, channel, userName) {
  ...
  for (var i = 1; i <= data.tracks.items.length; i++) {
    var trackName =
      data.tracks.items[i - 1].artists[0].name +
      ' - ' +
      data.tracks.items[i - 1].name;
...
function _searchSpotify(input, channel, userName, limit) {
  ...
  if (!data.tracks || !data.tracks.items || data.tracks.items.length === 0) {
    _slackMessage(
      'Sorry ' + userName + ', I could not find that track :(',
      channel.id
    );
    return;
  }
...
function _searchSpotifyPlaylist(input, channel, userName, limit) {
  ...
  if (
    ...
    _slackMessage(
      'Sorry ' + userName + ', I could not find that playlist :(',
      channel.id
    );
    return;
  }
...
function _searchSpotifyAlbum(input, channel, userName, limit) {
  ...
  if (!data.albums || !data.albums.items || data.albums.items.length === 0) {
    _slackMessage(
      'Sorry ' + userName + ', I could not find that album :(',
      channel.id
    );
    return;
  }
...
function _searchSpotifyArtist(input, channel, userName, limit) {
  ...
  if (!data.artists || !data.artists.items || data.artists.items.length === 0) {
    _slackMessage(
      'Sorry ' + userName + ', I could not find that artist :(',
      channel.id
    );
    return;
  }
...
function _theking(channel, userName) {
  ...
  _slackMessage(
    'King of Rock and Roll, just for you ' +
      userName +
      '\n```\n' +
      theking +
      '\n```\n',
...
function _searchalbum(input, channel) {
  ...
  if (data.albums && data.albums.items && data.albums.items.length > 0) {
    ...
    for (var i = 1; i <= data.albums.items.length; i++) {
      ...
      var trackName =
        data.albums.items[i - 1].artists[0].name +
        ' - ' +
        data.albums.items[i - 1].name;

      trackNames.push(trackName);
    }

    var message =
      'I found the following album(s):\n```\n' +
      trackNames.join('\n') +
  ...
  if (action === '') {
    ...
    if (action === 'add') {
      ...
      if (i !== -1) {
        ...
        message =
          'The user ' + username + ' has been removed from the blacklist.';
      } else {
